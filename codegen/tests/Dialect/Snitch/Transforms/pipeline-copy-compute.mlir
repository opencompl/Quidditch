...
func.func @test(%arg0: index, %extracted_slice : tensor<1x100xf64>, %14 : tensor<1200x400xf64>) -> tensor<1x1200xf64> {
  ...
  %24 = scf.for %arg2 = %c0 to %c1200 step %c40 iter_args(%arg3 = %arg1) -> (tensor<1x1200xf64>) {
    ...
    // CHECK: %[[RESULT0:.*]], %[[TOKEN0:.*]] = dma.start_tensor_copy of %[[ARG1]]
    // CHECK: %[[SLICE1:.*]] = tensor.extract_slice %[[ARG2]][%[[IV]], %[[ARG0]]]
    // CHECK: %[[RESULT1:.*]], %[[TOKEN1:.*]] = dma.start_tensor_copy of %[[SLICE1]]
    // CHECK: %[[SLICE2:.*]] = tensor.extract_slice %[[ITER]][0, %[[IV]]]
    // CHECK: %[[RESULT2:.*]], %[[TOKEN2:.*]] = dma.start_tensor_copy of %[[SLICE2]]
    ...
    %result_8, %token_9 = dma.start_tensor_copy of %extracted_slice to #quidditch_snitch.l1_encoding : tensor<1x100xf64> -> tensor<1x100xf64>
    %25 = dma.wait_for_tensor_copy of %extracted_slice : tensor<1x100xf64> to %result_8 using %token_9 -> tensor<1x100xf64>
    %result_10, %token_11 = dma.start_tensor_copy of %extracted_slice_6 to #quidditch_snitch.l1_encoding : tensor<40x100xf64> -> tensor<40x100xf64>
    %26 = dma.wait_for_tensor_copy of %extracted_slice_6 : tensor<40x100xf64> to %result_10 using %token_11 -> tensor<40x100xf64>
    %result_12, %token_13 = dma.start_tensor_copy of %extracted_slice_7 to #quidditch_snitch.l1_encoding : tensor<1x40xf64> -> tensor<1x40xf64>
    %27 = dma.wait_for_tensor_copy of %extracted_slice_7 : tensor<1x40xf64> to %result_12 using %token_13 -> tensor<1x40xf64>
    ...
    // CHECK: %[[OPA:.*]] = dma.wait_for_tensor_copy of %[[ARG1]]
    // CHECK-SAME: to %[[RESULT0]]
    // CHECK-SAME: using %[[TOKEN0]]
    // CHECK: %[[OPB:.*]] = dma.wait_for_tensor_copy of %[[SLICE1]]
    // CHECK-SAME: to %[[RESULT1]]
    // CHECK-SAME: using %[[TOKEN1]]
    // CHECK: %[[OPC:.*]] = dma.wait_for_tensor_copy of %[[SLICE2]]
